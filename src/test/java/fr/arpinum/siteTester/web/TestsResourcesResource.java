...
public class TestsResourcesResource {
	...
	public void canGet() throws IOException {
		...
		site.addResource("plop.html");
		Repositories.sites().add(site);
		final ClientResource resource = server.getRessource("/sites/www.arpinum.fr/resources");

		final Representation representation = resource.get();

		assertThat(resource.getStatus(), is(Status.SUCCESS_OK));
		assertThat(representation.getText(), containsString("plop.html"));
	}
}
